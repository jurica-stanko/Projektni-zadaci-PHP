<?php
$_naslov = "Administracija jela";
include("connect.php");

//dodaj jelo - gumb na koji ima pristup samo administrator
if (isset($_POST['_action_']) && $_POST['_action_'] == 'dodaj_jela') {
    ...
    } else if ($_SESSION['korisnik']['pravo'] == 'E') {
        $upit  = "INSERT INTO jela (naziv, opis, arhiva) VALUES ('$naslov', '$opis', '$arhiva')";
    }
    ...
    if (isset($_POST['submit_jelo'])) {
        ...
        if (!empty($imena)) {
            //prolazak kroz svaku sliku u polju, modificiranje imena, kopiranje u folder (jela) i dodavanje u bazu
            ...
            $_SESSION['message'] .= '<p>Uspješno ste dodali jelo</p>';
        }
    }
...
if (isset($_POST['_action_']) && $_POST['_action_'] == 'uredi_jela') {
    ...
    if ($_SESSION['korisnik']['pravo'] == 'A') {
        ...
        $upit  = "UPDATE jela SET naziv = '$naslov', opis = '$opis', arhiva = '$arhiva' WHERE id = $id LIMIT 1";
    }
    $rezultat = mysqli_query($baza, $upit);
...
if (isset($_GET['id']) && $_GET['id'] != '') {
    ...
    while ($_redak = mysqli_fetch_array($_rezultat)) {
        print '
                <div class = "pregled_wrapper">
                    <figure>
                        <img src="jela/' . $_redak['naziv'] . '" alt="' . $_redak['naziv'] . '" title="' . $_redak['naziv'] . '" class = "pregled_jela_img">
                    </figure>
                </div>
                ';
    }
    print '
			<h2>' . $redak['naziv'] . '</h2>
			' . $redak['opis'] . '
			<em><time datetime="' . $row['datum'] . '">' . ($redak['datum']) . '</time></em>
		</div>
...
//uređivanje jela forma
    ...
    print '
		...
		<form action="" class = "kontakt_forma" id="uredi_jela" method="POST" enctype="multipart/form-data">
            ...
            <input type="radio" name="arhiva" value="D"';
    if ($redak['arhiva'] == 'D') {
        echo ' checked="checked"';
        $arhiva_checked = TRUE;
    }
    echo ' /> DA &nbsp;&nbsp;
			<input type="radio" name="arhiva" value="N"';
    if ($arhiva_checked == FALSE) {
        echo ' checked="checked"';
    }
    echo ' /> NE
            <br>';

    //pravo odobravanja ima samo administrator
    if ($_SESSION['korisnik']['pravo'] == 'A') {
        print '
                <label for="odobreno">Odobreno:</label><br/>
                <input type="radio" name="odobreno" value="D"';
        if ($redak['odobreno'] == 'D') {
            echo ' checked="checked"';
            $odobreno_checked = TRUE;
        }
        echo ' /> DA &nbsp;&nbsp;
			    <input type="radio" name="odobreno" value="N"';
        if ($odobreno_checked == FALSE) {
            echo ' checked="checked"';
        }
        echo ' /> NE';
    }
